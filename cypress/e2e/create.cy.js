describe("auth spec", () => {
    ...
    it("admin can add employee", () => {
        ...
        cy.get("[data-id=btn-input]").click();
        cy.get("[data-id=field-name]").click({force:true}).type("Atmayanti");
        cy.get("[data-id=field-email]").click({force:true}).type("maya@mail.com");
        cy.get("[data-id=field-pwd]").click({force:true}).type("12345678");
        cy.get("[data-id=field-role]").select("Kasir", {force:true});
        cy.get("[data-id=field-birth-date]").click({force:true}).type("2002-12-12");
        cy.get("[data-id=field-address]").click({force:true}).type("Tulungagung");
        cy.get("[data-id=field-phone]").click({force:true}).type("083114514125");
